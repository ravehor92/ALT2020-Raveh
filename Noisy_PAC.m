	...
	for Episode = 1:Episodes
		...
		for Step = 1:Episode_Length
			...
			for Agent = 1:Num_of_Agents 
				...
				if num_of_samples(states(Agent),a_chosen) >= k  
					U(states(Agent), a_chosen, :, Current_Index(states(Agent),a_chosen)) = [s_new,R] ; 
					Current_Index(states(Agent),a_chosen) = mod(Current_Index(states(Agent),a_chosen) + 1,k+1) ;
					if Current_Index(states(Agent),a_chosen)==0
						Current_Index(states(Agent),a_chosen)=1 ; 
					end
					VI_flag = 1; 
				end
				states(Agent) = s_new ; 
